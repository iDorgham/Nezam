...
- **Subphase prompts** (`.nezam/core/plans/**/prompt.json`): validate optional `designSkillStack` against `.nezam/core/gates/design-skills.yaml`; suggest `phaseHint` from product type (`website` → `plan_design`, UI build → `develop_ui`, wireframes → `wireframe`)
