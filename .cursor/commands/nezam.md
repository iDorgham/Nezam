...
`.nezam/core/`, or `.nezam/core/scripts/` outside of this command. Changes made directly won't get
...
1. Check if a reference copy exists in `.nezam/core/` (for future upgrade path)
...
Workspace Paths — .nezam/workspace.paths.yaml
  ...
  nezam_root       .nezam/core/
  templates_root   .nezam/templates/
  hardlock_paths   .nezam/core/gates/hardlock-paths.json
...
4. Write back
5. If the key is `project.prd` → also update `.nezam/core/gates/hardlock-paths.json intake.prd`
...
node .nezam/core/scripts/checks/check-skill-frontmatter.js   # skill frontmatter
node .nezam/core/scripts/checks/check-sdd-swarm-integrity.js # SDD swarm
bash .nezam/core/scripts/checks/docs-layout-policy.sh # docs folder layout
```
...
NEZAM Workspace Upgrade
  ...
  Current version: 1.0.0 (.nezam/core/meta/VERSIONING.md)
