...
When using Drizzle ORM instead of Prisma:
- Prefer `drizzle-zod` for generating Zod schemas from database definitions.
- Ensure database migrations are tracked explicitly via `drizzle-kit generate`.
- Use Drizzle's relational queries for simpler and more readable data fetching.
