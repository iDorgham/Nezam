...
Build deterministic token manifests, theme scopes, and CSS-variable exports that satisfy token-first gates before `/DEVELOP`. Single-responsibility: design-system core (tokens). (Absorbs the previously-proposed `coi-design-system-core`.)
...
- Pairs with `@.cursor/skills/token-grid-typography/SKILL.md`, `@.cursor/skills/coi-component-library-api/SKILL.md`, `@.cursor/skills/coi-motion-3d-progressive/SKILL.md`, `@.cursor/skills/coi-a11y-automation/SKILL.md`.
- Enforces `[.cursor/rules/design-dev-gates.mdc](.cursor/rules/design-dev-gates.mdc)` Gate 1 and `[.cursor/rules/coia-design-gates-pro.mdc](.cursor/rules/coia-design-gates-pro.mdc)`.
