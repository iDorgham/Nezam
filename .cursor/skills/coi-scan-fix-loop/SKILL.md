...
# Purpose
Provide a repeatable remediation loop from `/SCAN` findings to verified fixes with documented status updates in `plan/INDEX.md`.

# Inputs
- `/SCAN` output payload (security, tests, perf, docs, a11y, content categories).
- Current branch diff and impacted files.
- `plan/INDEX.md` and active phase task files.
...
- Plan freshness: `plan/INDEX.md` reflects current scan state.

# Output Format
- Triage table: finding -> severity -> owner -> patch path -> status.
- Patch batch summary with verification commands and outcomes.
- Updated `plan/INDEX.md` entries for each resolved/deferred finding.
