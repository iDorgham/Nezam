# NEZAM Design Hub — Full Audit + Improvement Plan + Antigravity Prompt

> Audited: 2026-05-15  
> Auditor: Claude (Strategic Architecture Layer)  
> Scope: `.nezam/design-hub/` — full source scan

---

## Part 1: What the Design Hub Is

The NEZAM Design Hub is a **local Next.js 15 app** (running on port 4000) that serves as a **human-in-the-loop design decision engine**. It sits between design intent and code generation, producing two primary artifacts:
...
| **Gates** | `design-gates.mdc` (7 gates), `design-hub-gates.mdc` (wireframe lock gate) |
...
```
TASK: Improve the NEZAM Design Hub — Phase 1 (Foundation Repair) + Phase 2 (Feature Completion)

CONTEXT:
You are working inside the NEZAM monorepo. The Design Hub lives at `.nezam/design-hub/`. It is a Next.js 15 App Router project using Tailwind CSS 3 and Zustand. Its purpose is to be a human-in-the-loop design decision engine that produces two artifacts: DESIGN.md (design token contract) and wireframes_locked.json (page layout contract). These artifacts gate AI swarm development.

The following files are the critical ones you will modify:
- `.nezam/design-hub/lib/store/tokens.store.ts` — token state
- `.nezam/design-hub/lib/parsers/profile.parser.ts` — profile markdown parser
- `.nezam/design-hub/lib/locking/export-design.ts` — DESIGN.md generator
- `.nezam/design-hub/lib/token-injector.ts` — CSS variable injector
- `.nezam/design-hub/lib/paths.ts` — file path helpers
- `.nezam/design-hub/app/globals.css` — design server's own CSS tokens
- `.nezam/design-hub/app/api/lock/route.ts` — lock endpoint
- `.nezam/design-hub/app/api/ai/generate/route.ts` — AI wireframe generator
- `.nezam/design-hub/components/layout/Sidebar.tsx` — navigation
- `.nezam/design-hub/lib/store/session.store.ts` — session state (profile selection)
...
Scan all `.tsx` files in `.nezam/design-hub/` for direct hex usage in className strings. Replace:
...
```
You are a wireframe block layout generator for the NEZAM Design Hub.
...
After each file, verify TypeScript compiles with `pnpm tsc --noEmit` from the design-hub directory.
```
