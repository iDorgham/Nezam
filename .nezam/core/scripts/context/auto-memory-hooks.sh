...
health_file="$repo_root/.nezam/core/meta/HEALTH.md"
memory_file="$repo_root/.nezam/core/memory/MEMORY.md"
context_file="$repo_root/.nezam/core/memory/CONTEXT.md"
...
score_spec_completeness() {
  local files=(
    "$repo_root/.nezam/core/prd/PRD.md"
    "$repo_root/.nezam/core/prd/PROJECT_PROMPT.md"
    "$repo_root/docs/03_architecture/ARCHITECTURE.md"
    "$repo_root/.nezam/core/meta/VERSIONING.md"
...
install_hook_call() {
  if ! rg -q "auto-memory-hooks.sh --post-commit" "$post_commit_hook"; then
    {
      echo "bash \"\$repo_root/.nezam/core/scripts/context/auto-memory-hooks.sh\" --post-commit >/dev/null 2>&1 || true"
    } >>"$post_commit_hook"
  fi
}

build_companion_bundle() {
  local context_path="$repo_root/.nezam/core/memory/CONTEXT.md"
  local progress_path="$repo_root/docs/reports/progress/PROGRESS_REPORT.latest.md"
  local health_path="$repo_root/.nezam/core/meta/HEALTH.md"
...
case "$mode" in
  ...
  --bundle-report)
    ...
    ;;
  *)
    echo "Usage: bash .nezam/core/scripts/context/auto-memory-hooks.sh --install|--post-commit|--bundle-report"
    exit 1
    ;;
esac
