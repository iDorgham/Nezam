...
if [ -d docs/core ]; then
  echo "Moving docs/core to .nezam/core/prd..."
  mkdir -p .nezam/core/prd
  find docs/core -type f -exec mv -t .nezam/core/prd/ {} + 2>/dev/null || true
fi
...
find .cursor .nezam -name "*.md" -o -name "*.mdc" -o -name "*.yaml" | while read -r f; do
  ...
  if [ -f "$f" ]; then
    sed -i '' \
      ...
      -e 's|docs/core/required/|.nezam/core/prd/|g' \
