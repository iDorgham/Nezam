#!/usr/bin/env node
/**
 * Sync PRD.md §11 Release Roadmap table from .nezam/core/prd/release-roadmap.json
 *
 * Usage:
 *   node .nezam/core/scripts/prd/render-release-roadmap.mjs --write   # update PRD.md
 *   node .nezam/core/scripts/prd/render-release-roadmap.mjs --check  # exit 1 if drift
 */
import fs from 'fs';
...
const jsonPath = path.join(root, '.nezam/core/prd/release-roadmap.json');
const prdPath = path.join(root, '.nezam/core/prd/PRD.md');
...
if (!mode) {
  console.error('Usage: node .nezam/core/scripts/prd/render-release-roadmap.mjs --write | --check');
  process.exit(1);
}
...
if (mode === 'check') {
  if (current !== tableMd.trim()) {
    console.error(
      'PRD.md §11 release roadmap table is out of sync with .nezam/core/prd/release-roadmap.json'
    );
