...
Usage:
  node .nezam/core/scripts/sync/sync-ai-folders.js [options]
...
Examples:
  pnpm run ai:sync
  node .nezam/core/scripts/sync/sync-ai-folders.js --write
  node .nezam/core/scripts/sync/sync-ai-folders.js
...
for (const arg of rawArgs) {
  ...
  console.error(
    `sync-ai-folders: unknown option "${arg}"\n` +
      "  node .nezam/core/scripts/sync/sync-ai-folders.js --help\n" +
  ...
  for (const target of tool.targets || []) {
    ...
    for (const src of sourceFiles) {
      ...
      if (kind === "toml-commands") {
        ...
        const fileName = path.basename(src, ".md");
        const content = [
          "# generated by core/scripts/sync/sync-ai-folders.js",
          `name = "${fileName}"`,
  ...
  if (targetFilter && tools.length === 0) {
    console.error(
      `Unknown target tool: ${targetFilter}\n` +
        "  node .nezam/core/scripts/sync/sync-ai-folders.js --target=<tool-id> --write\n" +
        "  Valid ids are listed under tools[].id in .nezam/core/scripts/config/tools.config.json (e.g. claude, codex, gemini)."
    );
    process.exit(2);
  }
