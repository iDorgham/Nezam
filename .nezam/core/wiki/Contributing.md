...
**PRD release roadmap (§11):** Milestones live in [`.nezam/workspace/prd/release-roadmap.json`](../../.nezam/workspace/prd/release-roadmap.json). After you change that JSON, run **`pnpm prd:roadmap`** so the markdown table in [`.nezam/workspace/prd/PRD.md`](../../.nezam/workspace/prd/PRD.md) stays in sync—otherwise CI **`readiness`** fails on **`pnpm prd:roadmap:check`**. You can run **`pnpm prd:roadmap:check`** locally before pushing.
