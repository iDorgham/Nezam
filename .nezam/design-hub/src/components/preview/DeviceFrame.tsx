...
function buildVars(t: DesignTokens): React.CSSProperties {
  return {
    '--b':    t.colors.brand['500'],
    '--bh':   t.colors.brand['400'],
    '--bs':   t.colors.brand['100'],
    '--bd':   t.colors.brand['700'],
    '--bg':   t.colors.surface.bg,
    '--pn':   t.colors.surface.panel,
    '--br':   t.colors.surface.border,
    '--tx':   t.colors.text.primary,
    '--t2':   t.colors.text.secondary,
    '--mu':   t.colors.text.muted,
    '--di':   t.colors.text.disabled,
    '--ok':   t.colors.semantic.success,
    '--wa':   t.colors.semantic.warning,
    '--er':   t.colors.semantic.error,
    '--ac':   t.colors.accent['500'],
    '--rsm':  t.radius.sm,
    '--rmd':  t.radius.md,
    '--rlg':  t.radius.lg,
    '--rxl':  t.radius.xl,
    ...
    '--smd':  t.shadows.md,
    '--slg':  t.shadows.lg,
    '--ff':   t.typography.sans,
    '--fd':   t.typography.display,
    '--fm':   t.typography.mono,
    '--dur':  t.motion.duration.base,
    '--ease': t.motion.easing.default,
    '--bw':   t.borders.width,
    '--bs2':  t.borders.style,
    '--n1':   t.colors.neutral['100'],
    '--n2':   t.colors.neutral['200'],
    '--n3':   t.colors.neutral['300'],
    '--sp':   `${t.spacing.base}px`,
...
const css = {
  ...
  nav: (isMobile: boolean): React.CSSProperties => ({
    ...
    borderBottom: 'var(--bw) var(--bs2) var(--br)',
    boxShadow: 'var(--ssm)',
  ...
  logo: (): React.CSSProperties => ({
    fontSize: '15px', fontWeight: 700, fontFamily: 'var(--fd)', color: 'var(--b)',
  ...
  navLink: (): React.CSSProperties => ({
    fontSize: '13px', color: 'var(--mu)', cursor: 'pointer',
  }),
  pill: (active = false): React.CSSProperties => ({
    ...
    backgroundColor: active ? 'var(--b)' : 'var(--bs)',
  ...
  btnPrimary: (isMobile = false): React.CSSProperties => ({
    ...
    backgroundColor: 'var(--b)', color: '#fff',
    borderRadius: 'var(--rmd)', fontSize: '13px', fontWeight: 600,
    border: 'none', cursor: 'pointer', boxShadow: 'var(--smd)',
  }),
  btnOutline: (isMobile = false): React.CSSProperties => ({
    display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
    padding: isMobile ? '9px 18px' : '10px 22px',
    backgroundColor: 'transparent', color: 'var(--t2)',
    borderRadius: 'var(--rmd)', fontSize: '13px', fontWeight: 500,
    border: 'var(--bw) var(--bs2) var(--br)', cursor: 'pointer',
  }),
  card: (): React.CSSProperties => ({
    backgroundColor: 'var(--pn)',
    border: 'var(--bw) var(--bs2) var(--br)',
  ...
  input: (): React.CSSProperties => ({
    ...
    padding: '0 12px', fontSize: '13px', color: 'var(--di)',
    display: 'flex', alignItems: 'center',
  }),
  label: (): React.CSSProperties => ({
    fontSize: '11px', fontWeight: 500, color: 'var(--mu)', marginBottom: '5px',
  }),
  h1: (isMobile: boolean, isTablet: boolean): React.CSSProperties => ({
    fontSize: isMobile ? '26px' : isTablet ? '34px' : '44px',
    fontWeight: 800, fontFamily: 'var(--fd)', lineHeight: 1.1,
    color: 'var(--tx)', marginBottom: '14px', letterSpacing: '-0.02em',
  }),
  h2: (isMobile: boolean): React.CSSProperties => ({
    fontSize: isMobile ? '20px' : '26px',
    fontWeight: 700, fontFamily: 'var(--fd)', lineHeight: 1.2,
    color: 'var(--tx)', marginBottom: '8px', letterSpacing: '-0.015em',
  }),
  ...
  footer: (): React.CSSProperties => ({
    padding: '32px 28px',
    backgroundColor: 'var(--pn)',
    borderTop: 'var(--bw) var(--bs2) var(--br)',
  }),
}
...
function NavBar({
  ...
  return (
    <nav style={css.nav(isMobile)}>
      <div style={css.logo()}>
        <span style={{ width: 20, height: 20, borderRadius: '5px', backgroundColor: 'var(--b)', display: 'inline-block' }} />
        YourApp
      </div>
      {!isMobile && (
...
function AppSidebar({ isMobile, items, active = 0 }: { isMobile: boolean; items: string[]; active?: number }) {
  ...
  return (
    <aside style={{
      ...
      backgroundColor: 'var(--pn)',
      borderRight: 'var(--bw) var(--bs2) var(--br)',
      display: 'flex', flexDirection: 'column',
      padding: '14px 8px',
    }}>
      <div style={{ ...css.logo(), fontSize: '13px', padding: '4px 8px', marginBottom: '14px' }}>
        <span style={{ width: 16, height: 16, borderRadius: '4px', backgroundColor: 'var(--b)', display: 'inline-block' }} />
        YourApp
      </div>
      {items.map((item, i) => (
        <div key={item} style={{
          ...
          color: i === active ? 'var(--b)' : 'var(--mu)',
          backgroundColor: i === active ? 'var(--bs)' : 'transparent',
          marginBottom: '1px',
          cursor: 'pointer',
          display: 'flex', alignItems: 'center', gap: '8px',
        }}>
          <span style={{ width: 6, height: 6, borderRadius: '50%', backgroundColor: i === active ? 'var(--b)' : 'var(--n3)', flexShrink: 0 }} />
          {item}
        </div>
      ))}
    </aside>
...
function StatCard({ label, value, delta, up }: { label: string; value: string; delta: string; up: boolean }) {
  return (
    <div style={{ ...css.card(), padding: '16px' }}>
      <p style={{ fontSize: '11px', color: 'var(--mu)', marginBottom: '6px' }}>{label}</p>
      <p style={{ fontSize: '22px', fontWeight: 700, color: 'var(--tx)', marginBottom: '4px', fontFamily: 'var(--fd)', letterSpacing: '-0.02em' }}>{value}</p>
      <p style={{ fontSize: '11px', color: up ? 'var(--ok)' : 'var(--er)', display: 'flex', alignItems: 'center', gap: '3px' }}>
        <span>{up ? '↑' : '↓'}</span> {delta}
      </p>
    </div>
  )
...
function InputField({ label, placeholder, type = 'text' }: { label: string; placeholder: string; type?: string }) {
  return (
    <div>
      ...
      <div style={css.input()}>
        <span style={{ fontSize: '13px', color: 'var(--di)' }}>{placeholder}</span>
      </div>
    </div>
  )
}
  ...
  return (
    <footer style={css.footer()}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
        ...
        <p style={{ fontSize: '11px', color: 'var(--mu)' }}>
          © 2025 YourApp · Privacy · Terms · Support
        </p>
        {!minimal && (
          <div style={{ display: 'flex', gap: '16px' }}>
            {['Twitter', 'GitHub', 'LinkedIn'].map((s) => (
              <span key={s} style={{ fontSize: '11px', color: 'var(--mu)', cursor: 'pointer' }}>{s}</span>
            ))}
          </div>
        )}
      </div>
  ...
  return (
    <div style={css.page(isMobile)}>
      ...
      <section style={{
        ...
        background: `linear-gradient(170deg, var(--bs) 0%, var(--bg) 60%)`,
        textAlign: 'center',
        ...
        <p style={{ fontSize: isMobile ? '14px' : '16px', color: 'var(--mu)', maxWidth: '480px', margin: '0 auto 28px', lineHeight: 1.65 }}>
        ...
        <div style={{ marginTop: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex' }}>
            {['#3b82f6','#10b981','#8b5cf6'].map((c, i) => (
              <div key={i} style={{ width: 26, height: 26, borderRadius: '50%', backgroundColor: c, border: '2px solid var(--bg)', marginLeft: i ? '-6px' : 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '9px', color: '#fff', fontWeight: 700 }}>
                {['YD','AB','CX'][i]}
              </div>
            ))}
          </div>
          <span style={{ fontSize: '12px', color: 'var(--mu)' }}>Trusted by 12,000+ teams</span>
          <span style={{ fontSize: '12px', color: 'var(--wa)' }}>★★★★★ 4.9/5</span>
        </div>
      </section>

      {/* Logos strip */}
      <div style={{ borderTop: 'var(--bw) var(--bs2) var(--br)', borderBottom: 'var(--bw) var(--bs2) var(--br)', padding: '18px 28px', display: 'flex', justifyContent: 'center', gap: '32px', flexWrap: 'wrap' }}>
        {['Acme Corp', 'Globex', 'Initech', 'Umbrella', 'Hooli'].map((n) => (
          <span key={n} style={{ fontSize: '12px', fontWeight: 600, color: 'var(--n3)', letterSpacing: '0.04em', textTransform: 'uppercase' }}>{n}</span>
        ))}
      </div>
      ...
      <section style={css.section(isMobile)}>
        <div style={{ marginBottom: '36px' }}>
          ...
          <p style={{ fontSize: '14px', color: 'var(--mu)' }}>Powerful features that grow with your team</p>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: '14px' }}>
          {features.map((f, i) => (
            <div key={f.title} style={{ ...css.card(), padding: '20px' }}>
              <div style={{ width: 32, height: 32, borderRadius: 'var(--rmd)', backgroundColor: 'var(--bs)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '12px' }}>
                <span style={{ width: 14, height: 14, borderRadius: '3px', backgroundColor: 'var(--b)', opacity: 0.7 + i * 0.05 }} />
              </div>
              <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--tx)', marginBottom: '6px' }}>{f.title}</p>
              <p style={{ fontSize: '12px', color: 'var(--mu)', lineHeight: 1.6 }}>{f.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* CTA banner */}
      <section style={{ margin: '0 28px 48px', padding: '36px', backgroundColor: 'var(--b)', borderRadius: 'var(--rxl)', textAlign: 'center' }}>
        <h2 style={{ fontSize: '22px', fontWeight: 700, color: '#fff', marginBottom: '10px', fontFamily: 'var(--fd)' }}>Ready to get started?</h2>
        <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.75)', marginBottom: '20px' }}>Join thousands of teams building better products.</p>
        <button style={{ ...css.btnPrimary(), backgroundColor: '#fff', color: 'var(--b)' }}>Start free trial</button>
      </section>
...
function PricingPage({ isMobile, isTablet }: { isMobile: boolean; isTablet: boolean }) {
  const plans = [
    { name: 'Starter',    price: '$0',   period: '/mo', color: 'var(--n2)', tag: null,     features: ['5 projects', '3 team members', '1 GB storage', 'Email support'] },
    { name: 'Pro',        price: '$29',  period: '/mo', color: 'var(--b)',  tag: 'Popular', features: ['Unlimited projects', '25 members', '50 GB storage', 'Priority support', 'Analytics', 'Custom domain'] },
    { name: 'Enterprise', price: '$99',  period: '/mo', color: 'var(--bd)', tag: null,     features: ['Everything in Pro', 'Unlimited members', '1 TB storage', 'Dedicated support', 'SSO & SAML', 'SLA 99.99%'] },
  ]
  return (
    <div style={css.page(isMobile)}>
      <NavBar isMobile={isMobile} />
      <section style={{ ...css.section(isMobile), textAlign: 'center' }}>
        ...
        <p style={{ fontSize: '14px', color: 'var(--mu)', marginBottom: '36px' }}>Start free. Scale as you grow. No surprises.</p>
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : isTablet ? 'repeat(2,1fr)' : 'repeat(3,1fr)', gap: '16px', maxWidth: '820px', margin: '0 auto' }}>
          {plans.map((plan) => (
            <div key={plan.name} style={{ ...css.card(), padding: '28px', border: plan.name === 'Pro' ? `2px solid var(--b)` : undefined, position: 'relative' }}>
              {plan.tag && (
                <div style={{ position: 'absolute', top: '-12px', left: '50%', transform: 'translateX(-50%)', ...css.pill(true) }}>{plan.tag}</div>
              )}
              <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--mu)', marginBottom: '8px' }}>{plan.name}</p>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '2px', marginBottom: '20px' }}>
                <span style={{ fontSize: '36px', fontWeight: 800, fontFamily: 'var(--fd)', color: 'var(--tx)', letterSpacing: '-0.03em' }}>{plan.price}</span>
                <span style={{ fontSize: '13px', color: 'var(--mu)' }}>{plan.period}</span>
              </div>
              <button style={{ ...css.btnPrimary(), width: '100%', backgroundColor: plan.name === 'Pro' ? 'var(--b)' : 'transparent', color: plan.name === 'Pro' ? '#fff' : 'var(--tx)', border: plan.name === 'Pro' ? 'none' : 'var(--bw) var(--bs2) var(--br)', marginBottom: '20px' }}>
                Get started
              </button>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {plan.features.map((f) => (
                  <div key={f} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px', color: 'var(--t2)' }}>
                    <span style={{ color: 'var(--ok)', fontWeight: 700 }}>✓</span> {f}
                  </div>
                ))}
              </div>
            </div>
  ...
  return (
    <div style={css.page(isMobile)}>
      ...
      <section style={css.section(isMobile)}>
        ...
        <p style={{ fontSize: '14px', color: 'var(--mu)', marginBottom: '32px' }}>Ideas, engineering notes, and product updates from our team.</p>
        {/* Tags */}
          ...
          {posts.map((post) => (
            <div key={post.title} style={{ ...css.card(), padding: '22px', cursor: 'pointer' }}>
              <div style={{ marginBottom: '12px' }}>
                <span style={{ fontSize: '10px', fontWeight: 600, color: 'var(--b)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{post.tag}</span>
              </div>
              <p style={{ fontSize: '15px', fontWeight: 700, color: 'var(--tx)', marginBottom: '8px', lineHeight: 1.35, fontFamily: 'var(--fd)' }}>{post.title}</p>
              <p style={{ fontSize: '12px', color: 'var(--mu)', lineHeight: 1.6, marginBottom: '16px' }}>{post.excerpt}</p>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <div style={{ width: 24, height: 24, borderRadius: '50%', backgroundColor: 'var(--b)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '9px', color: '#fff', fontWeight: 700 }}>YD</div>
                  <span style={{ fontSize: '11px', color: 'var(--mu)' }}>{post.date}</span>
                </div>
                <span style={{ fontSize: '11px', color: 'var(--mu)' }}>{post.read} read</span>
              </div>
            </div>
          ))}
        </div>
    ...
    <div style={css.page(isMobile)}>
      ...
      <article style={{ maxWidth: '680px', margin: '0 auto', padding: isMobile ? '32px 16px' : '48px 28px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
          ...
          <span style={{ fontSize: '11px', color: 'var(--mu)' }}>May 2025 · 8 min read</span>
        </div>
        <h1 style={{ fontSize: isMobile ? '24px' : '34px', fontWeight: 800, fontFamily: 'var(--fd)', lineHeight: 1.15, color: 'var(--tx)', marginBottom: '16px', letterSpacing: '-0.02em' }}>{page.name}</h1>
        <p style={{ fontSize: '16px', color: 'var(--t2)', lineHeight: 1.7, marginBottom: '24px' }}>
          Everything your team needs to move fast, stay aligned, and ship great products. No complexity, just results.
        </p>
        {/* Author row */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '14px 0', borderTop: 'var(--bw) var(--bs2) var(--br)', borderBottom: 'var(--bw) var(--bs2) var(--br)', marginBottom: '28px' }}>
          <div style={{ width: 32, height: 32, borderRadius: '50%', backgroundColor: 'var(--b)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', color: '#fff', fontWeight: 700 }}>YD</div>
          <div>
            <p style={{ fontSize: '12px', fontWeight: 600, color: 'var(--tx)' }}>Yasser Dorgham</p>
            <p style={{ fontSize: '11px', color: 'var(--mu)' }}>Design Architect</p>
          </div>
        </div>
        {/* Body paragraphs */}
        {[
          ...
          <p key={i} style={{ fontSize: '14px', color: 'var(--t2)', lineHeight: 1.8, marginBottom: '18px' }}>{p}</p>
        ))}
        {/* Code block */}
        <div style={{ backgroundColor: '#1a1b26', borderRadius: 'var(--rmd)', padding: '16px', marginBottom: '18px', fontFamily: 'var(--fm)' }}>
...
function AuthPage({ page, isMobile }: { page: ArchPage; isMobile: boolean }) {
  ...
  return (
    <div style={{
      ...
      background: `linear-gradient(135deg, var(--bs) 0%, var(--bg) 55%)`,
    }}>
      {!isMobile && (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '48px', backgroundColor: 'var(--b)' }}>
          <div style={css.logo()}>
          ...
          <div style={{ marginTop: '48px' }}>
            <h2 style={{ fontSize: '28px', fontWeight: 700, color: '#fff', marginBottom: '12px', fontFamily: 'var(--fd)', lineHeight: 1.2 }}>
      ...
      <div style={{ flex: isMobile ? 1 : '0 0 420px', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: isMobile ? '32px 16px' : '48px 40px', backgroundColor: 'var(--bg)' }}>
        <div style={{ width: '100%', maxWidth: '340px' }}>
          <div style={{ marginBottom: '28px' }}>
            <div style={{ ...css.logo(), marginBottom: '20px' }}>
              <span style={{ width: 18, height: 18, borderRadius: '4px', backgroundColor: 'var(--b)', display: 'inline-block' }} />
              YourApp
            </div>
            <h2 style={{ fontSize: '20px', fontWeight: 700, color: 'var(--tx)', marginBottom: '4px', fontFamily: 'var(--fd)' }}>{title}</h2>
            <p style={{ fontSize: '13px', color: 'var(--mu)' }}>{subtitle}</p>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            {isLogin && (
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <span style={{ fontSize: '12px', color: 'var(--b)', cursor: 'pointer' }}>Forgot password?</span>
              </div>
            )}
            ...
            {!isForgot && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', margin: '4px 0' }}>
                <div style={{ flex: 1, height: '1px', backgroundColor: 'var(--br)' }} />
                <span style={{ fontSize: '11px', color: 'var(--mu)' }}>or continue with</span>
                <div style={{ flex: 1, height: '1px', backgroundColor: 'var(--br)' }} />
              </div>
            )}
            {!isForgot && (
            ...
            <p style={{ textAlign: 'center', fontSize: '12px', color: 'var(--mu)' }}>
              {isLogin ? "Don't have an account? " : isForgot ? 'Remember it? ' : 'Already have one? '}
              <span style={{ color: 'var(--b)', cursor: 'pointer', fontWeight: 500 }}>
...
function DashboardPage({ page, isMobile }: { page: ArchPage; isMobile: boolean }) {
  ...
  const statusColors: Record<string, string> = { Active: 'var(--ok)', Pending: 'var(--wa)', Inactive: 'var(--mu)' }
  const statusBg: Record<string, string>     = { Active: 'var(--ok)18', Pending: 'var(--wa)18', Inactive: 'var(--n1)' }
    ...
    <div style={{ ...css.page(isMobile), display: 'flex', height: '100%', minHeight: '700px' }}>
      ...
      <main style={{ flex: 1, overflow: 'hidden', padding: isMobile ? '16px' : '22px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <h2 style={{ fontSize: '16px', fontWeight: 700, color: 'var(--tx)', fontFamily: 'var(--fd)', letterSpacing: '-0.01em' }}>{page.name}</h2>
            <p style={{ fontSize: '11px', color: 'var(--mu)', marginTop: '2px' }}>Welcome back — here's what's happening today.</p>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '14px', flex: 1 }}>
          ...
          <div style={{ ...css.card(), padding: '18px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--tx)' }}>Revenue Overview</p>
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-end', height: '90px' }}>
              <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '100%' }}>
                {['12k','6k','0'].map((v) => <span key={v} style={{ fontSize: '9px', color: 'var(--mu)', width: '22px' }}>{v}</span>)}
              </div>
              <div style={{ flex: 1, display: 'flex', alignItems: 'flex-end', gap: '5px', height: '100%' }}>
                {[42,58,45,72,61,88,65,80,55,90,70,95].map((h, i) => (
                  <div key={i} style={{ flex: 1, height: `${h}%`, backgroundColor: i === 11 ? 'var(--b)' : `var(--b)${Math.round(30 + i * 5).toString(16)}`, borderRadius: '3px 3px 0 0' }} />
                ))}
              </div>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', paddingLeft: '30px' }}>
              {['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'].map((m) => (
                <span key={m} style={{ fontSize: '8px', color: 'var(--mu)' }}>{m}</span>
              ))}
            </div>
          </div>
          ...
          <div style={{ ...css.card(), overflow: 'hidden' }}>
            <div style={{ padding: '14px 16px', borderBottom: 'var(--bw) var(--bs2) var(--br)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--tx)' }}>Recent Users</p>
              <span style={{ fontSize: '11px', color: 'var(--b)', cursor: 'pointer' }}>View all →</span>
            </div>
            {rows.map((row, i) => (
              <div key={row.name} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 14px', borderBottom: i < rows.length - 1 ? 'var(--bw) var(--bs2) var(--br)' : 'none' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: '12px', fontWeight: 500, color: 'var(--tx)', lineHeight: 1.2 }}>{row.name}</p>
                  <p style={{ fontSize: '10px', color: 'var(--mu)' }}>{row.role}</p>
                </div>
                <span style={{ fontSize: '10px', fontWeight: 500, color: statusColors[row.status], backgroundColor: statusBg[row.status], padding: '2px 7px', borderRadius: 'var(--rf)' }}>
                  {row.status}
                </span>
              </div>
            ))}
  ...
  return (
    <div style={{ ...css.page(isMobile), display: 'flex', minHeight: '700px' }}>
      ...
      <main style={{ flex: 1, padding: isMobile ? '16px' : '22px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h2 style={{ fontSize: '16px', fontWeight: 700, color: 'var(--tx)', fontFamily: 'var(--fd)' }}>{page.name}</h2>
            <p style={{ fontSize: '11px', color: 'var(--mu)', marginTop: '2px' }}>Last 30 days · Updated 2 minutes ago</p>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '3fr 2fr', gap: '14px', flex: 1 }}>
          <div style={{ ...css.card(), padding: '18px' }}>
            <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--tx)', marginBottom: '14px' }}>Traffic Overview</p>
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: '3px', height: '100px' }}>
              {[30,45,38,62,50,75,58,80,65,90,70,85,60,95,72,88,78,92,68,84,73,90,65,88,78,92,70,86,80,95].map((h, i) => (
                <div key={i} style={{ flex: 1, height: `${h}%`, backgroundColor: i >= 28 ? 'var(--b)' : `var(--b)30`, borderRadius: '2px 2px 0 0' }} />
              ))}
            </div>
          </div>
          <div style={{ ...css.card(), padding: '18px' }}>
            <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--tx)', marginBottom: '14px' }}>Top Pages</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {topPages.map((tp) => (
                <div key={tp.path}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                    <span style={{ fontSize: '11px', color: 'var(--t2)', fontFamily: 'var(--fm)' }}>{tp.path}</span>
                    <span style={{ fontSize: '11px', color: 'var(--mu)' }}>{tp.views}</span>
                  </div>
                  <div style={{ height: '4px', backgroundColor: 'var(--n2)', borderRadius: '2px' }}>
                    <div style={{ height: '100%', width: `${tp.pct}%`, backgroundColor: 'var(--b)', borderRadius: '2px' }} />
                  </div>
                </div>
              ))}
            </div>
    ...
    <div style={{ ...css.page(isMobile), display: 'flex', minHeight: '700px' }}>
      ...
      <main style={{ flex: 1, padding: isMobile ? '16px' : '28px', overflow: 'hidden' }}>
        <div style={{ maxWidth: '520px' }}>
          <h2 style={{ fontSize: '16px', fontWeight: 700, color: 'var(--tx)', marginBottom: '4px', fontFamily: 'var(--fd)' }}>{page.name}</h2>
          <p style={{ fontSize: '12px', color: 'var(--mu)', marginBottom: '28px' }}>Manage your account preferences and settings.</p>

          {/* Avatar section */}
          <div style={{ ...css.card(), padding: '20px', marginBottom: '16px' }}>
            <p style={{ fontSize: '12px', fontWeight: 600, color: 'var(--tx)', marginBottom: '14px' }}>Profile Photo</p>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
              <div style={{ width: 56, height: 56, borderRadius: '50%', backgroundColor: 'var(--b)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', color: '#fff', fontWeight: 700 }}>YD</div>
              <div style={{ display: 'flex', gap: '8px' }}>
                <button style={{ ...css.btnOutline(), padding: '6px 14px', fontSize: '12px' }}>Upload photo</button>
                <button style={{ ...css.btnOutline(), padding: '6px 14px', fontSize: '12px', color: 'var(--er)', borderColor: 'var(--er)30' }}>Remove</button>
              </div>
            </div>
          </div>
          ...
          {/* Form section */}
          <div style={{ ...css.card(), padding: '20px', marginBottom: '16px' }}>
            <p style={{ fontSize: '12px', fontWeight: 600, color: 'var(--tx)', marginBottom: '16px' }}>Personal Information</p>
          ...
          {/* Danger zone */}
          <div style={{ ...css.card(), padding: '20px', border: '1px solid var(--er)20' }}>
            <p style={{ fontSize: '12px', fontWeight: 600, color: 'var(--er)', marginBottom: '6px' }}>Danger Zone</p>
            <p style={{ fontSize: '12px', color: 'var(--mu)', marginBottom: '14px' }}>Permanently delete your account and all associated data.</p>
            <button style={{ ...css.btnOutline(), padding: '7px 16px', fontSize: '12px', color: 'var(--er)', borderColor: 'var(--er)40' }}>Delete account</button>
          </div>
        </div>
      </main>
    </div>
...
function ProfilePage({ page, isMobile, isTablet }: { page: ArchPage; isMobile: boolean; isTablet: boolean }) {
  ...
  return (
    <div style={css.page(isMobile)}>
      ...
      {/* Hero */}
      <div style={{ background: `linear-gradient(160deg, var(--bs) 0%, var(--bg) 50%)`, padding: isMobile ? '40px 16px 32px' : '60px 28px 40px' }}>
        <div style={{ display: 'flex', alignItems: isMobile ? 'flex-start' : 'center', gap: isMobile ? '16px' : '24px', flexDirection: isMobile ? 'column' : 'row' }}>
          <div style={{ width: isMobile ? 64 : 80, height: isMobile ? 64 : 80, borderRadius: '50%', backgroundColor: 'var(--b)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: isMobile ? '22px' : '28px', color: '#fff', fontWeight: 800, flexShrink: 0 }}>YD</div>
          <div style={{ flex: 1 }}>
            <h1 style={{ fontSize: isMobile ? '22px' : '28px', fontWeight: 800, color: 'var(--tx)', fontFamily: 'var(--fd)', marginBottom: '4px', letterSpacing: '-0.02em' }}>{page.name}</h1>
            <p style={{ fontSize: '14px', color: 'var(--mu)', marginBottom: '12px' }}>Design Architect · San Francisco, CA</p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '16px' }}>
              {[{ n: '8+', l: 'Years exp.' }, { n: '40+', l: 'Projects' }, { n: '12k', l: 'GitHub stars' }].map(({ n, l }) => (
                <div key={l} style={{ textAlign: 'center' }}>
                  <p style={{ fontSize: '22px', fontWeight: 800, color: 'var(--tx)', fontFamily: 'var(--fd)', letterSpacing: '-0.02em' }}>{n}</p>
                  <p style={{ fontSize: '11px', color: 'var(--mu)' }}>{l}</p>
                </div>
              ))}
            </div>
          )}
          ...
          {works.map((w) => (
            <div key={w.title} style={{ ...css.card(), overflow: 'hidden', cursor: 'pointer' }}>
              <div style={{ height: '100px', backgroundColor: w.color + '18', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <div style={{ width: 40, height: 40, borderRadius: 'var(--rmd)', backgroundColor: w.color + '30', border: `2px solid ${w.color}40` }} />
              </div>
              <div style={{ padding: '14px' }}>
                <span style={{ fontSize: '10px', fontWeight: 600, color: w.color, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{w.tag}</span>
                <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--tx)', marginTop: '4px' }}>{w.title}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section style={css.section(isMobile)}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
          <div>
            ...
            <p style={{ fontSize: '12px', color: 'var(--mu)' }}>128 products</p>
          </div>
          <div style={{ display: 'flex', gap: '8px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: '16px' }}>
          {products.map((p) => (
            <div key={p.name} style={{ cursor: 'pointer' }}>
              <div style={{ height: isMobile ? '130px' : '160px', backgroundColor: p.color + '25', borderRadius: 'var(--rlg)', marginBottom: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'var(--bw) var(--bs2) var(--br)', position: 'relative', overflow: 'hidden' }}>
                <div style={{ width: 56, height: 56, borderRadius: 'var(--rmd)', backgroundColor: p.color + '50' }} />
                {p.tag && (
                  <div style={{ position: 'absolute', top: '8px', left: '8px', ...css.pill(p.tag === 'Sale'), fontSize: '9px', padding: '2px 7px', backgroundColor: p.tag === 'Sale' ? 'var(--er)' : p.tag === 'New' ? 'var(--ok)' : 'var(--b)', color: '#fff' }}>{p.tag}</div>
                )}
              </div>
              <p style={{ fontSize: '13px', fontWeight: 500, color: 'var(--tx)', marginBottom: '3px' }}>{p.name}</p>
              <p style={{ fontSize: '13px', fontWeight: 700, color: 'var(--b)' }}>{p.price}</p>
            </div>
          ))}
        </div>
      </section>
    ...
    <div style={css.page(isMobile)}>
      ...
      <section style={css.section(isMobile)}>
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '32px' }}>
          {/* Image */}
          <div>
            <div style={{ height: isMobile ? '220px' : '320px', backgroundColor: 'var(--n1)', borderRadius: 'var(--rlg)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'var(--bw) var(--bs2) var(--br)', marginBottom: '12px' }}>
              <div style={{ width: 80, height: 80, borderRadius: 'var(--rlg)', backgroundColor: 'var(--n2)' }} />
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '8px' }}>
              {['var(--n1)','var(--bs)','var(--n2)','var(--n1)'].map((bg, i) => (
                <div key={i} style={{ height: '56px', backgroundColor: bg, borderRadius: 'var(--rmd)', border: i === 0 ? '2px solid var(--b)' : 'var(--bw) var(--bs2) var(--br)', cursor: 'pointer' }} />
              ))}
            </div>
          </div>
          {/* Info */}
          <div>
            <div style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
              ...
              <span style={{ fontSize: '11px', color: 'var(--ok)', display: 'flex', alignItems: 'center', gap: '4px' }}>● In stock</span>
            </div>
            <h1 style={{ fontSize: isMobile ? '22px' : '28px', fontWeight: 800, color: 'var(--tx)', fontFamily: 'var(--fd)', marginBottom: '8px', letterSpacing: '-0.02em' }}>{page.name}</h1>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px', marginBottom: '14px' }}>
              <span style={{ fontSize: '24px', fontWeight: 800, color: 'var(--b)', fontFamily: 'var(--fd)' }}>$249</span>
              <span style={{ fontSize: '14px', color: 'var(--mu)', textDecoration: 'line-through' }}>$319</span>
              <span style={{ fontSize: '12px', color: 'var(--er)', fontWeight: 600 }}>22% off</span>
            </div>
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                {['#d4a57a','#7ab8d4','#a8d4a0','#b0a0d4'].map((c, i) => (
                  <div key={c} style={{ width: 28, height: 28, borderRadius: '50%', backgroundColor: c, border: i === 0 ? '3px solid var(--b)' : '2px solid var(--br)', cursor: 'pointer' }} />
                ))}
              </div>
            </div>
            {/* Quantity */}
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', border: 'var(--bw) var(--bs2) var(--br)', borderRadius: 'var(--rmd)', overflow: 'hidden' }}>
                {['−','1','+'].map((v, i) => (
                  <button key={i} style={{ width: 36, height: 36, border: 'none', backgroundColor: i === 1 ? 'var(--bg)' : 'var(--pn)', color: 'var(--tx)', cursor: 'pointer', fontSize: '14px', borderLeft: i > 0 ? 'var(--bw) var(--bs2) var(--br)' : 'none' }}>{v}</button>
                ))}
              </div>
            </div>
    ...
    <div style={css.page(isMobile)}>
      ...
      <section style={css.section(isMobile)}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 320px', gap: '24px' }}>
          <div>
            {items.map((item, i) => (
              <div key={item.name} style={{ display: 'flex', gap: '14px', padding: '16px 0', borderBottom: 'var(--bw) var(--bs2) var(--br)' }}>
                <div style={{ width: 72, height: 72, borderRadius: 'var(--rmd)', backgroundColor: item.color + '25', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'var(--bw) var(--bs2) var(--br)' }}>
                  <div style={{ width: 28, height: 28, borderRadius: 'var(--rsm)', backgroundColor: item.color + '60' }} />
                </div>
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--tx)', marginBottom: '4px' }}>{item.name}</p>
                  <p style={{ fontSize: '12px', color: 'var(--mu)', marginBottom: '10px' }}>Color: Natural</p>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', border: 'var(--bw) var(--bs2) var(--br)', borderRadius: 'var(--rsm)', overflow: 'hidden' }}>
                      {['−', item.qty.toString(), '+'].map((v, j) => (
                        <button key={j} style={{ width: 28, height: 28, border: 'none', backgroundColor: j === 1 ? 'transparent' : 'var(--n1)', color: 'var(--tx)', cursor: 'pointer', fontSize: '12px', borderLeft: j > 0 ? 'var(--bw) var(--bs2) var(--br)' : 'none' }}>{v}</button>
                      ))}
                    </div>
                    <span style={{ fontSize: '12px', color: 'var(--mu)', cursor: 'pointer' }}>Remove</span>
                  </div>
                </div>
                <p style={{ fontSize: '14px', fontWeight: 700, color: 'var(--tx)', flexShrink: 0 }}>${item.price * item.qty}</p>
              </div>
            ))}
          </div>
          {/* Summary */}
          <div style={{ ...css.card(), padding: '22px', height: 'fit-content' }}>
            <p style={{ fontSize: '14px', fontWeight: 700, color: 'var(--tx)', marginBottom: '16px' }}>Order Summary</p>
            {[['Subtotal', `$${subtotal}`], ['Shipping', 'Free'], ['Tax', `$${Math.round(subtotal * 0.08)}`]].map(([l, v]) => (
              <div key={l} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                <span style={{ fontSize: '12px', color: 'var(--mu)' }}>{l}</span>
                <span style={{ fontSize: '12px', color: 'var(--tx)' }}>{v}</span>
              </div>
            ))}
            <div style={{ borderTop: 'var(--bw) var(--bs2) var(--br)', paddingTop: '10px', display: 'flex', justifyContent: 'space-between', marginBottom: '16px' }}>
              <span style={{ fontSize: '13px', fontWeight: 700, color: 'var(--tx)' }}>Total</span>
              <span style={{ fontSize: '16px', fontWeight: 800, color: 'var(--tx)', fontFamily: 'var(--fd)' }}>${subtotal + Math.round(subtotal * 0.08)}</span>
            </div>
      ...
      <section style={css.section(isMobile)}>
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 340px', gap: '28px', maxWidth: '860px', margin: '0 auto' }}>
              ...
              {['Contact', 'Shipping', 'Payment', 'Review'].map((step, i) => (
                <div key={step} style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <div style={{ width: 20, height: 20, borderRadius: '50%', backgroundColor: i === 0 ? 'var(--b)' : i < 0 ? 'var(--ok)' : 'var(--n2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '9px', color: i <= 0 ? '#fff' : 'var(--mu)', fontWeight: 700 }}>{i + 1}</div>
                    <span style={{ fontSize: '11px', color: i === 0 ? 'var(--b)' : 'var(--mu)', fontWeight: i === 0 ? 600 : 400 }}>{step}</span>
                  </div>
                  {i < 3 && <div style={{ flex: 1, height: '1px', backgroundColor: 'var(--n2)', margin: '0 6px' }} />}
                </div>
              ))}
            </div>
            {/* Form */}
            <div style={{ ...css.card(), padding: '22px' }}>
              <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--tx)', marginBottom: '16px' }}>Contact Information</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                  <InputField label="First name" placeholder="Yasser" />
                  <InputField label="Last name" placeholder="Dorgham" />
              ...
              <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--tx)', margin: '20px 0 14px' }}>Shipping Address</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          <div style={{ ...css.card(), padding: '20px', height: 'fit-content' }}>
            <p style={{ fontSize: '13px', fontWeight: 700, color: 'var(--tx)', marginBottom: '14px' }}>Order (3 items)</p>
            {[['Minimal Chair ×1','$249'],['Arc Table Lamp ×2','$298'],['Ceramic Vase ×1','$65']].map(([n,p]) => (
              <div key={n as string} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                <span style={{ fontSize: '11px', color: 'var(--mu)', maxWidth: '160px' }}>{n}</span>
                <span style={{ fontSize: '11px', color: 'var(--tx)', fontWeight: 500 }}>{p}</span>
              </div>
            ))}
            <div style={{ borderTop: 'var(--bw) var(--bs2) var(--br)', paddingTop: '10px', marginTop: '6px', display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ fontSize: '13px', fontWeight: 700, color: 'var(--tx)' }}>Total</span>
              <span style={{ fontSize: '15px', fontWeight: 800, color: 'var(--tx)', fontFamily: 'var(--fd)' }}>$659</span>
            </div>
          </div>
        </div>
      </section>
  ...
  return (
    <div style={css.page(isMobile)}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: isMobile ? '60px 16px' : '80px 28px', textAlign: 'center' }}>
        <div style={{ width: 72, height: 72, borderRadius: '50%', backgroundColor: 'var(--ok)18', border: `2px solid var(--ok)40`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '20px' }}>
          <span style={{ fontSize: '28px', color: 'var(--ok)' }}>✓</span>
        </div>
        <h1 style={{ fontSize: '28px', fontWeight: 800, color: 'var(--tx)', fontFamily: 'var(--fd)', marginBottom: '10px', letterSpacing: '-0.02em' }}>Order confirmed!</h1>
        <p style={{ fontSize: '14px', color: 'var(--mu)', maxWidth: '400px', lineHeight: 1.6, marginBottom: '28px' }}>
        ...
        <div style={{ ...css.card(), padding: '20px', maxWidth: '380px', width: '100%', marginBottom: '24px', textAlign: 'left' }}>
          {[['Order number','#2025-0481'],['Estimated delivery','May 28–30, 2025'],['Shipping to','San Francisco, CA'],['Email sent to','yasser@example.com']].map(([l,v]) => (
            <div key={l as string} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px', borderBottom: 'var(--bw) var(--bs2) var(--br)', paddingBottom: '10px' }}>
              <span style={{ fontSize: '11px', color: 'var(--mu)' }}>{l}</span>
              <span style={{ fontSize: '11px', color: 'var(--tx)', fontWeight: 500 }}>{v}</span>
            </div>
          ))}
        </div>
        <div style={{ display: 'flex', gap: '10px' }}>
...
function OnboardingPage({ page, isMobile }: { page: ArchPage; isMobile: boolean }) {
  ...
  return (
    <div style={{ ...css.page(isMobile), display: 'flex', flexDirection: 'column', minHeight: '600px', backgroundColor: 'var(--b)' }}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: isMobile ? '40px 20px' : '60px 40px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '6px', marginBottom: '40px' }}>
          {steps.slice(0, -1).map((_, i) => (
            <div key={i} style={{ width: i === activeStep ? 24 : 6, height: 6, borderRadius: 'var(--rf)', backgroundColor: i === activeStep ? '#fff' : 'rgba(255,255,255,0.3)', transition: '0.3s' }} />
          ))}
        </div>
        <div style={{ fontSize: '48px', marginBottom: '20px' }}>{content.icon}</div>
        <h1 style={{ fontSize: isMobile ? '24px' : '30px', fontWeight: 800, color: '#fff', fontFamily: 'var(--fd)', marginBottom: '12px', letterSpacing: '-0.02em' }}>{content.title}</h1>
        <p style={{ fontSize: '15px', color: 'rgba(255,255,255,0.75)', maxWidth: '360px', lineHeight: 1.65, marginBottom: '36px' }}>{content.desc}</p>
        <button style={{ ...css.btnPrimary(), backgroundColor: '#fff', color: 'var(--b)', padding: '12px 28px', fontSize: '14px', marginBottom: '16px' }}>{content.action}</button>
  ...
  return (
    <div style={css.page(isMobile)}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: isMobile ? '60px 16px' : '80px 28px', textAlign: 'center' }}>
        <p style={{ fontSize: '80px', fontWeight: 900, fontFamily: 'var(--fd)', color: 'var(--n2)', letterSpacing: '-0.04em', lineHeight: 1, marginBottom: '16px' }}>404</p>
        <h1 style={{ fontSize: isMobile ? '20px' : '26px', fontWeight: 700, color: 'var(--tx)', fontFamily: 'var(--fd)', marginBottom: '10px' }}>Page not found</h1>
        <p style={{ fontSize: '14px', color: 'var(--mu)', maxWidth: '380px', lineHeight: 1.6, marginBottom: '28px' }}>
        ...
        <div style={{ marginTop: '40px', textAlign: 'left', width: '100%', maxWidth: '340px' }}>
          <p style={{ fontSize: '12px', fontWeight: 600, color: 'var(--mu)', marginBottom: '10px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Helpful links</p>
          {['Documentation', 'API Reference', 'Community Forum', 'Status Page'].map((l) => (
            <div key={l} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 0', borderBottom: 'var(--bw) var(--bs2) var(--br)' }}>
              <span style={{ fontSize: '13px', color: 'var(--b)', cursor: 'pointer' }}>{l}</span>
    ...
    <div style={css.page(isMobile)}>
      ...
      <section style={{ ...css.section(isMobile), maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: isMobile ? '32px' : '48px' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', backgroundColor: 'var(--bs)', borderRadius: 'var(--rf)', padding: '4px 12px', marginBottom: '14px' }}>
            <div style={{ width: 6, height: 6, borderRadius: '50%', backgroundColor: 'var(--ok)' }} />
            <span style={{ fontSize: '11px', fontWeight: 600, color: 'var(--b)' }}>Available for projects</span>
          </div>
          <h1 style={css.h1(isMobile, isTablet)}>Let&apos;s work together</h1>
          <p style={{ fontSize: isMobile ? '13px' : '15px', color: 'var(--t2)', maxWidth: '440px', margin: '0 auto', lineHeight: 1.6 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <div>
                ...
                <div style={{ ...css.input(), paddingRight: '8px', justifyContent: 'space-between' }}>
                  ...
                  <span style={{ color: 'var(--mu)', fontSize: '10px' }}>▾</span>
                </div>
              </div>
              {/* Message textarea */}
              <div>
                <p style={css.label()}>Message</p>
                <div style={{ backgroundColor: 'var(--bg)', border: 'var(--bw) var(--bs2) var(--br)', borderRadius: 'var(--rmd)', padding: '10px 12px', height: '100px', fontSize: '13px', color: 'var(--di)' }}>
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px' }}>
                  {['< $5k', '$5–20k', '$20–50k', '$50k+'].map((b, i) => (
                    <div key={b} style={{ textAlign: 'center', padding: '7px 4px', borderRadius: 'var(--rmd)', border: 'var(--bw) var(--bs2) var(--br)', fontSize: '11px', fontWeight: i === 1 ? 600 : 400, backgroundColor: i === 1 ? 'var(--bs)' : 'transparent', color: i === 1 ? 'var(--b)' : 'var(--mu)', cursor: 'pointer' }}>{b}</div>
                  ))}
                </div>
              </div>
              ...
              <p style={{ fontSize: '11px', color: 'var(--mu)', textAlign: 'center' }}>
          ...
          {!isMobile && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ ...css.card(), padding: '20px' }}>
                <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--tx)', marginBottom: '16px' }}>Contact info</p>
                {[
                  ...
                  <div key={row.label} style={{ display: 'flex', gap: '12px', marginBottom: '14px' }}>
                    <div style={{ width: 32, height: 32, borderRadius: 'var(--rmd)', backgroundColor: 'var(--bs)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: '14px' }}>
                      {row.icon}
                    </div>
                    <div>
                      <p style={{ fontSize: '11px', color: 'var(--mu)', marginBottom: '2px' }}>{row.label}</p>
                      <p style={{ fontSize: '13px', fontWeight: 500, color: 'var(--tx)' }}>{row.value}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {/* Social links */}
              <div style={{ ...css.card(), padding: '20px' }}>
                <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--tx)', marginBottom: '14px' }}>Connect</p>
                {['Twitter / X', 'LinkedIn', 'GitHub', 'Dribbble'].map((soc, i) => (
                  <div key={soc} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 0', borderBottom: i < 3 ? 'var(--bw) var(--bs2) var(--br)' : 'none' }}>
                    <span style={{ fontSize: '13px', color: 'var(--b)', cursor: 'pointer' }}>{soc}</span>
                    <span style={{ fontSize: '11px', color: 'var(--mu)' }}>→</span>
                  </div>
                ))}
              </div>

              {/* Availability badge */}
              <div style={{ borderRadius: 'var(--rlg)', backgroundColor: 'var(--ok)', padding: '16px 20px' }}>
      ...
      <div style={{ display: 'flex', minHeight: 'calc(100% - 52px)' }}>
        ...
        <main style={{ flex: 1, padding: isMobile ? '20px 16px' : '28px', overflowY: 'auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '24px', gap: '12px' }}>
            <div>
              <h1 style={{ fontSize: isMobile ? '20px' : '24px', fontWeight: 700, fontFamily: 'var(--fd)', color: 'var(--tx)', letterSpacing: '-0.02em', marginBottom: '4px' }}>Team</h1>
              <p style={{ fontSize: '13px', color: 'var(--mu)' }}>{members.length} members across {depts.length - 1} departments</p>
            </div>
            <button style={{ ...css.btnPrimary(), fontSize: '12px', padding: '8px 16px' }}>+ Invite member</button>
          </div>

          {/* Dept filter tabs */}
          <div style={{ display: 'flex', gap: '4px', marginBottom: '20px', borderBottom: 'var(--bw) var(--bs2) var(--br)', paddingBottom: '0' }}>
            {depts.map((d, i) => (
              <button key={d} style={{ padding: '6px 14px', fontSize: '12px', fontWeight: i === 0 ? 600 : 400, color: i === 0 ? 'var(--b)' : 'var(--mu)', background: 'none', border: 'none', borderBottom: i === 0 ? '2px solid var(--b)' : '2px solid transparent', cursor: 'pointer', marginBottom: '-1px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
            ...
            <div style={{ display: 'flex', gap: '2px', borderRadius: 'var(--rmd)', border: 'var(--bw) var(--bs2) var(--br)', overflow: 'hidden' }}>
              {['Grid', 'List'].map((v, i) => (
                <button key={v} style={{ padding: '6px 12px', fontSize: '11px', backgroundColor: i === 0 ? 'var(--bs)' : 'transparent', color: i === 0 ? 'var(--b)' : 'var(--mu)', border: 'none', cursor: 'pointer' }}>{v}</button>
              ))}
            </div>
          </div>
            ...
            {members.map((m) => (
              <div key={m.name} style={{ ...css.card(), padding: '18px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--tx)', lineHeight: 1.2, marginBottom: '2px' }}>{m.name}</p>
                    <p style={{ fontSize: '11px', color: 'var(--mu)' }}>{m.role}</p>
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: '10px', fontWeight: 500, color: 'var(--b)', backgroundColor: 'var(--bs)', padding: '2px 8px', borderRadius: 'var(--rf)' }}>{m.dept}</span>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                    <div style={{ width: 6, height: 6, borderRadius: '50%', backgroundColor: statusColor[m.status as keyof typeof statusColor] }} />
                    <span style={{ fontSize: '10px', color: 'var(--mu)' }}>{m.status}</span>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: '6px' }}>
...
function FeaturesPage({ page, isMobile, isTablet }: { page: ArchPage; isMobile: boolean; isTablet: boolean }) {
  const features = [
    { title: 'Lightning Fast', body: 'Sub-100ms response times worldwide with global edge delivery. Your users experience near-instant interactions regardless of location.', accent: 'var(--b)' },
    { title: 'Built-in Analytics', body: 'Real-time dashboards, funnel analysis, and retention cohorts. Understand your users deeply without switching tools.', accent: 'var(--ac)' },
    { title: 'Team Collaboration', body: 'Live presence indicators, shared workspaces, and granular permissions. Keep everyone aligned at any scale.', accent: 'var(--ok)' },
    { title: 'Enterprise Security', body: 'SOC 2 Type II certified, end-to-end encryption, and SSO/SAML support. Your data is always protected.', accent: 'var(--wa)' },
  ]
  const accentShapes = ['var(--b)', 'var(--ac)', 'var(--ok)', 'var(--er)']
  ...
  return (
    <div style={css.page(isMobile)}>
      ...
      <section style={{ ...css.section(isMobile), textAlign: 'center', paddingBottom: isMobile ? '32px' : '48px' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', backgroundColor: 'var(--bs)', borderRadius: 'var(--rf)', padding: '4px 14px', marginBottom: '16px' }}>
          <div style={{ width: 6, height: 6, borderRadius: '50%', backgroundColor: 'var(--b)' }} />
          <span style={{ fontSize: '11px', fontWeight: 600, color: 'var(--b)' }}>Everything you need to ship faster</span>
        </div>
        <h1 style={{ ...css.h1(isMobile, isTablet), maxWidth: '560px', margin: '0 auto 16px' }}>
          Powerful features for<br />modern teams
        </h1>
        <p style={{ fontSize: isMobile ? '13px' : '15px', color: 'var(--t2)', maxWidth: '440px', margin: '0 auto 28px', lineHeight: 1.7 }}>
        ...
        <div style={{ display: 'flex', gap: isMobile ? '16px' : '40px', justifyContent: 'center', marginTop: '44px', flexWrap: 'wrap' }}>
          {[['10k+', 'Active users'], ['99.9%', 'Uptime SLA'], ['< 50ms', 'Avg latency'], ['SOC 2', 'Certified']].map(([stat, label]) => (
            <div key={label} style={{ textAlign: 'center' }}>
              <p style={{ fontSize: isMobile ? '20px' : '26px', fontWeight: 800, fontFamily: 'var(--fd)', color: 'var(--b)', letterSpacing: '-0.02em' }}>{stat}</p>
              <p style={{ fontSize: '11px', color: 'var(--mu)', marginTop: '2px' }}>{label}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {features.map((feat, i) => (
        <section
          ...
          style={{
            ...css.section(isMobile),
            backgroundColor: i % 2 === 1 ? 'var(--pn)' : 'transparent',
            borderTop: 'var(--bw) var(--bs2) var(--br)',
            borderBottom: 'var(--bw) var(--bs2) var(--br)',
          }}
        >
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? '24px' : '48px', alignItems: 'center', maxWidth: '860px', margin: '0 auto', direction: (i % 2 === 1 && !isMobile) ? 'rtl' : 'ltr' }}>
            <div style={{ direction: 'ltr' }}>
              <div style={{ display: 'inline-flex', width: 44, height: 44, borderRadius: 'var(--rlg)', backgroundColor: feat.accent + '20', alignItems: 'center', justifyContent: 'center', marginBottom: '16px' }}>
                {/* Inline SVG icon */}
              ...
              <p style={{ fontSize: '13px', color: 'var(--t2)', lineHeight: 1.7, marginBottom: '20px' }}>{feat.body}</p>
              <div style={{ display: 'flex', gap: '8px' }}>
            ...
            {/* Visual mockup */}
            <div style={{ direction: 'ltr', borderRadius: 'var(--rlg)', border: 'var(--bw) var(--bs2) var(--br)', backgroundColor: 'var(--pn)', padding: '20px', minHeight: isMobile ? '120px' : '160px', boxShadow: 'var(--smd)', display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <div style={{ display: 'flex', gap: '6px', marginBottom: '4px' }}>
                {['#ff5f57','#febc2e','#28c840'].map(c => <div key={c} style={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: c }} />)}
              </div>
              {[...Array(3)].map((_, j) => (
                <div key={j} style={{ height: j === 0 ? '10px' : '7px', borderRadius: 'var(--rf)', backgroundColor: j === 0 ? feat.accent + '40' : 'var(--br)', width: ['80%','60%','70%'][j] }} />
              ))}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginTop: '4px' }}>
                {[...Array(4)].map((_, j) => <div key={j} style={{ height: '40px', borderRadius: 'var(--rmd)', backgroundColor: 'var(--bg)', border: 'var(--bw) var(--bs2) var(--br)' }} />)}
              </div>
            </div>
          </div>
        </section>
      ...
      {/* CTA */}
      <section style={{ ...css.section(isMobile), textAlign: 'center', backgroundColor: 'var(--b)', color: '#fff' }}>
        <h2 style={{ fontSize: isMobile ? '22px' : '30px', fontWeight: 800, fontFamily: 'var(--fd)', marginBottom: '10px' }}>Ready to get started?</h2>
        <p style={{ fontSize: '14px', opacity: 0.85, marginBottom: '24px' }}>Join 10,000+ teams building with {page.name || 'YourApp'}.</p>
        <button style={{ padding: '12px 28px', backgroundColor: '#fff', color: 'var(--b)', borderRadius: 'var(--rmd)', fontSize: '14px', fontWeight: 700, border: 'none', cursor: 'pointer', boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }}>
          Start free today →
        </button>
      </section>
      <Footer />
...
function DocsPage({ page, isMobile }: { page: ArchPage; isMobile: boolean }) {
  ...
  return (
    <div style={css.page(isMobile)}>
      {/* Docs topbar */}
      <nav style={{ ...css.nav(isMobile), backgroundColor: 'var(--pn)', borderBottom: 'var(--bw) var(--bs2) var(--br)' }}>
        <div style={css.logo()}>
          <span style={{ width: 18, height: 18, borderRadius: '4px', backgroundColor: 'var(--b)', display: 'inline-block' }} />
        ...
        {!isMobile && (
          <div style={{ flex: 1, maxWidth: '280px', margin: '0 24px' }}>
            <div style={{ ...css.input(), fontSize: '12px' }}>
              <span style={{ color: 'var(--mu)', marginRight: '6px' }}>⌘K</span>
      ...
      <div style={{ display: 'flex', minHeight: 'calc(100% - 52px)' }}>
        {/* Left TOC sidebar */}
        {!isMobile && (
          <aside style={{ width: '220px', flexShrink: 0, backgroundColor: 'var(--pn)', borderRight: 'var(--bw) var(--bs2) var(--br)', padding: '20px 0', overflowY: 'auto' }}>
            {/* Version badge */}
            <div style={{ margin: '0 16px 16px', padding: '6px 10px', borderRadius: 'var(--rmd)', backgroundColor: 'var(--bs)', border: 'var(--bw) var(--bs2) var(--br)' }}>
              <p style={{ fontSize: '10px', color: 'var(--mu)', marginBottom: '1px' }}>Version</p>
              <p style={{ fontSize: '12px', fontWeight: 600, color: 'var(--b)' }}>v2.4.0 — Latest</p>
            </div>
            {chapters.map((ch, ci) => (
              <div key={ch} style={{ marginBottom: '4px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 16px', cursor: 'pointer' }}>
                  <span style={{ fontSize: '10px', color: 'var(--mu)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{ch}</span>
                </div>
                {ci === 0 && sections.map((s, si) => (
                  <div key={s} style={{ padding: '4px 16px 4px 28px', cursor: 'pointer', backgroundColor: si === 2 ? 'var(--bs)' : 'transparent', borderLeft: si === 2 ? '2px solid var(--b)' : '2px solid transparent', marginLeft: '0' }}>
                    <span style={{ fontSize: '12px', color: si === 2 ? 'var(--b)' : 'var(--mu)', fontWeight: si === 2 ? 600 : 400 }}>{s}</span>
                  </div>
                ))}
              </div>
            ))}
        ...
        <main style={{ flex: 1, padding: isMobile ? '20px 16px' : '32px 40px', maxWidth: '760px', lineHeight: 1.7 }}>
          ...
          <div style={{ display: 'flex', gap: '6px', alignItems: 'center', marginBottom: '20px' }}>
            {['Docs', 'Getting Started', 'Quick Start'].map((seg, i) => (
              <span key={seg} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                {i > 0 && <span style={{ color: 'var(--mu)', fontSize: '11px' }}>/</span>}
                <span style={{ fontSize: '11px', color: i === 2 ? 'var(--tx)' : 'var(--b)', cursor: i < 2 ? 'pointer' : 'default' }}>{seg}</span>
              </span>
            ))}
          </div>

          {/* Title */}
          <h1 style={{ fontSize: '28px', fontWeight: 800, fontFamily: 'var(--fd)', color: 'var(--tx)', letterSpacing: '-0.02em', marginBottom: '8px' }}>
            {page.name || 'Quick Start'}
          </h1>
          <p style={{ fontSize: '15px', color: 'var(--t2)', marginBottom: '28px', lineHeight: 1.6 }}>
            Get up and running with {page.name || 'YourApp'} in under five minutes.
          </p>

          {/* Callout */}
          <div style={{ borderRadius: 'var(--rmd)', border: 'var(--bw) var(--bs2) var(--b)', backgroundColor: 'var(--bs)', padding: '14px 16px', marginBottom: '24px', display: 'flex', gap: '10px' }}>
            <svg viewBox="0 0 16 16" width={16} height={16} style={{ flexShrink: 0, marginTop: '2px', color: 'var(--b)' }}>
            ...
            <p style={{ fontSize: '13px', color: 'var(--b)', lineHeight: 1.6 }}>
              <strong>Prerequisites:</strong> Node.js ≥ 18.x and an active YourApp account.
            </p>
          </div>

          {/* Step 1 */}
          <h2 style={{ fontSize: '18px', fontWeight: 700, fontFamily: 'var(--fd)', color: 'var(--tx)', marginBottom: '10px', marginTop: '28px' }}>1. Install the SDK</h2>
          <p style={{ fontSize: '13px', color: 'var(--t2)', marginBottom: '12px' }}>Install via npm, yarn, or pnpm:</p>

          {/* Code block */}
          <div style={{ borderRadius: 'var(--rmd)', backgroundColor: '#0f1117', padding: '16px 20px', marginBottom: '24px', overflowX: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
              <span style={{ fontSize: '10px', color: '#64748b', fontFamily: 'monospace' }}>Terminal</span>
              <span style={{ fontSize: '10px', color: 'var(--b)', cursor: 'pointer' }}>Copy</span>
            </div>
            {codeSnippet.split('\n').map((line, i) => (
          ...
          {/* Step 2 */}
          <h2 style={{ fontSize: '18px', fontWeight: 700, fontFamily: 'var(--fd)', color: 'var(--tx)', marginBottom: '10px' }}>2. Configure your environment</h2>
          <p style={{ fontSize: '13px', color: 'var(--t2)', marginBottom: '16px', lineHeight: 1.7 }}>
            Add your API key to your environment variables. Never commit secrets to source control — use a <span style={{ color: 'var(--b)', cursor: 'pointer' }}>.env file</span> or your platform&apos;s secret manager.
          </p>

          {/* Nav between articles */}
          <div style={{ display: 'flex', gap: '12px', marginTop: '40px', paddingTop: '24px', borderTop: 'var(--bw) var(--bs2) var(--br)' }}>
            <div style={{ ...css.card(), flex: 1, padding: '14px', cursor: 'pointer' }}>
              <p style={{ fontSize: '10px', color: 'var(--mu)', marginBottom: '4px' }}>← Previous</p>
              <p style={{ fontSize: '12px', fontWeight: 600, color: 'var(--b)' }}>Installation</p>
            </div>
            <div style={{ ...css.card(), flex: 1, padding: '14px', cursor: 'pointer', textAlign: 'right' }}>
              <p style={{ fontSize: '10px', color: 'var(--mu)', marginBottom: '4px' }}>Next →</p>
              <p style={{ fontSize: '12px', fontWeight: 600, color: 'var(--b)' }}>Configuration</p>
            </div>
          </div>
        </main>
        ...
        {!isMobile && (
          <aside style={{ width: '200px', flexShrink: 0, padding: '32px 20px', borderLeft: 'var(--bw) var(--bs2) var(--br)' }}>
            <p style={{ fontSize: '11px', fontWeight: 600, color: 'var(--mu)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '12px' }}>On this page</p>
            {['Install the SDK', 'Configure env', 'First request', 'Next steps'].map((h, i) => (
              <div key={h} style={{ padding: '5px 0', borderLeft: i === 0 ? '2px solid var(--b)' : '2px solid transparent', paddingLeft: '10px', marginLeft: '-10px', cursor: 'pointer' }}>
                <span style={{ fontSize: '12px', color: i === 0 ? 'var(--b)' : 'var(--mu)' }}>{h}</span>
              </div>
            ))}
          </aside>
        )}
  ...
  return (
    <div style={css.page(isMobile)}>
      <nav style={{ ...css.nav(isMobile) }}>
        ...
        {!isMobile && (
          <div style={{ display: 'flex', gap: '6px', padding: '3px', borderRadius: 'var(--rmd)', backgroundColor: 'var(--bg)', border: 'var(--bw) var(--bs2) var(--br)' }}>
            {['v2.4', 'v2.3', 'v2.0'].map((v, i) => (
              <button key={v} style={{ padding: '4px 10px', borderRadius: 'var(--rmd)', fontSize: '11px', backgroundColor: i === 0 ? 'var(--pn)' : 'transparent', color: i === 0 ? 'var(--tx)' : 'var(--mu)', border: 'none', cursor: 'pointer' }}>{v}</button>
            ))}
          </div>
        )}
        <button style={{ ...css.btnPrimary(isMobile), fontSize: '12px', padding: '6px 14px' }}>Get API Key</button>
      ...
      <div style={{ display: 'flex', minHeight: 'calc(100% - 52px)' }}>
        {/* Left endpoint list */}
        {!isMobile && (
          <aside style={{ width: '240px', flexShrink: 0, backgroundColor: 'var(--pn)', borderRight: 'var(--bw) var(--bs2) var(--br)', padding: '16px 0', overflowY: 'auto' }}>
            {/* Auth header */}
            <div style={{ padding: '0 16px 12px', borderBottom: 'var(--bw) var(--bs2) var(--br)', marginBottom: '12px' }}>
              <p style={{ fontSize: '10px', fontWeight: 700, color: 'var(--mu)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '6px' }}>Authentication</p>
              <div style={{ ...css.input(), fontSize: '11px', backgroundColor: 'var(--bg)' }}>sk-…••••••••</div>
            </div>
            {['Users', 'Projects', 'Teams', 'Webhooks', 'Events'].map((group, gi) => (
              <div key={group} style={{ marginBottom: '4px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 16px', cursor: 'pointer' }}>
                  <span style={{ fontSize: '11px', fontWeight: 600, color: gi === 0 ? 'var(--tx)' : 'var(--mu)' }}>{group}</span>
                  <span style={{ fontSize: '10px', color: 'var(--mu)' }}>{gi === 0 ? '5' : gi === 1 ? '4' : '3'}</span>
                </div>
                {gi === 0 && endpoints.slice(0, 5).map((ep, ei) => (
                  <div key={ei} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '4px 16px 4px 28px', cursor: 'pointer', backgroundColor: ei === 0 ? 'var(--bs)' : 'transparent' }}>
                    ...
                    <span style={{ fontSize: '10px', color: ei === 0 ? 'var(--b)' : 'var(--mu)', fontFamily: 'monospace' }}>/users{ep.path.replace('/v1/users', '')}</span>
                  </div>
                ))}
              </div>
            ))}
        ...
        <main style={{ flex: 1, padding: isMobile ? '16px' : '28px 32px', overflowY: 'auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px', flexWrap: 'wrap' }}>
            <span style={{ fontSize: '12px', fontWeight: 700, color: methodColor['GET'], backgroundColor: methodBg['GET'], padding: '4px 10px', borderRadius: 'var(--rmd)' }}>GET</span>
            <code style={{ fontSize: '14px', fontFamily: 'monospace', color: 'var(--tx)', backgroundColor: 'var(--pn)', padding: '4px 10px', borderRadius: 'var(--rmd)', border: 'var(--bw) var(--bs2) var(--br)' }}>/v1/users</code>
            <span style={{ fontSize: '11px', color: 'var(--ok)', display: 'flex', alignItems: 'center', gap: '4px' }}>
              <div style={{ width: 6, height: 6, borderRadius: '50%', backgroundColor: 'var(--ok)' }} /> 200 OK
            </span>
          </div>
          <p style={{ fontSize: '14px', color: 'var(--t2)', marginBottom: '24px', lineHeight: 1.6 }}>
          ...
          <div style={{ ...css.card(), overflow: 'hidden', marginBottom: '20px' }}>
            <div style={{ padding: '12px 16px', borderBottom: 'var(--bw) var(--bs2) var(--br)', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <p style={{ fontSize: '12px', fontWeight: 700, color: 'var(--tx)' }}>Query Parameters</p>
              <span style={{ fontSize: '10px', color: 'var(--mu)', backgroundColor: 'var(--bg)', padding: '1px 6px', borderRadius: 'var(--rf)' }}>4 params</span>
            </div>
            {[
              ...
              <div key={param.name} style={{ display: 'grid', gridTemplateColumns: '120px 70px 1fr', gap: '12px', padding: '10px 16px', borderBottom: i < 3 ? 'var(--bw) var(--bs2) var(--br)' : 'none', alignItems: 'start' }}>
                <code style={{ fontSize: '12px', color: 'var(--b)', fontFamily: 'monospace' }}>{param.name}</code>
                <span style={{ fontSize: '10px', color: 'var(--mu)', backgroundColor: 'var(--bg)', padding: '2px 6px', borderRadius: 'var(--rf)', alignSelf: 'start' }}>{param.type}</span>
                <p style={{ fontSize: '12px', color: 'var(--t2)' }}>{param.desc}</p>
              </div>
            ))}
          </div>

          {/* Response example */}
          <p style={{ fontSize: '12px', fontWeight: 700, color: 'var(--tx)', marginBottom: '10px' }}>Response</p>
          <div style={{ borderRadius: 'var(--rmd)', backgroundColor: '#0f1117', padding: '16px 20px' }}>
      ...
      <div style={{ display: 'flex', minHeight: 'calc(100% - 52px)' }}>
        ...
        <main style={{ flex: 1, padding: isMobile ? '16px' : '24px', overflowY: 'auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
            <div>
              <h1 style={{ fontSize: '20px', fontWeight: 700, fontFamily: 'var(--fd)', color: 'var(--tx)', letterSpacing: '-0.02em', marginBottom: '3px' }}>Media Library</h1>
              <p style={{ fontSize: '12px', color: 'var(--mu)' }}>23 files · 48.6 MB used</p>
            </div>
          ...
          {/* Upload drop zone */}
          <div style={{ border: '2px dashed var(--br)', borderRadius: 'var(--rlg)', padding: '20px', textAlign: 'center', marginBottom: '20px', backgroundColor: 'var(--pn)' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px' }}>
              <svg viewBox="0 0 24 24" width={28} height={28} fill="none" style={{ color: 'var(--mu)' }}>
                <path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M17 8l-5-5-5 5M12 3v12" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
              <p style={{ fontSize: '13px', fontWeight: 500, color: 'var(--tx)' }}>Drop files to upload</p>
              <p style={{ fontSize: '11px', color: 'var(--mu)' }}>or <span style={{ color: 'var(--b)', cursor: 'pointer' }}>browse files</span> · max 50 MB per file</p>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(auto-fill, minmax(160px, 1fr))', gap: '12px' }}>
            {files.map((f) => (
              <div key={f.name} style={{ ...css.card(), overflow: 'hidden', cursor: 'pointer' }}>
                {/* Preview area */}
                <div style={{ height: '100px', backgroundColor: f.color + '15', display: 'flex', alignItems: 'center', justifyContent: 'center', borderBottom: 'var(--bw) var(--bs2) var(--br)', position: 'relative' }}>
                  {f.type === 'image' && (
                ...
                <div style={{ padding: '8px 10px' }}>
                  <p style={{ fontSize: '11px', fontWeight: 500, color: 'var(--tx)', marginBottom: '2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{f.name}</p>
                  <p style={{ fontSize: '10px', color: 'var(--mu)' }}>{f.size}</p>
                </div>
              </div>
            ))}
          </div>
    ...
    <div style={css.page(isMobile)}>
      ...
      <div style={{ display: 'flex', minHeight: 'calc(100% - 52px)' }}>
        ...
        <main style={{ flex: 1, padding: isMobile ? '16px' : '28px', maxWidth: '700px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <h1 style={{ fontSize: '20px', fontWeight: 700, fontFamily: 'var(--fd)', color: 'var(--tx)', letterSpacing: '-0.015em' }}>Notifications</h1>
              {unreadCount > 0 && (
                <span style={{ fontSize: '11px', fontWeight: 700, color: '#fff', backgroundColor: 'var(--b)', padding: '2px 8px', borderRadius: 'var(--rf)' }}>{unreadCount} new</span>
              )}
            </div>
            <div style={{ display: 'flex', gap: '8px' }}>
          ...
          {/* Filter tabs */}
          <div style={{ display: 'flex', gap: '0', borderBottom: 'var(--bw) var(--bs2) var(--br)', marginBottom: '16px' }}>
            {['All', 'Unread', 'Mentions', 'Tasks'].map((tab, i) => (
              <button key={tab} style={{ padding: '7px 16px', fontSize: '12px', fontWeight: i === 0 ? 600 : 400, color: i === 0 ? 'var(--b)' : 'var(--mu)', background: 'none', border: 'none', borderBottom: i === 0 ? '2px solid var(--b)' : '2px solid transparent', cursor: 'pointer', marginBottom: '-1px' }}>{tab}</button>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
            {notifs.map((n, i) => (
              <div key={i} style={{ display: 'flex', gap: '14px', padding: '14px 16px', borderRadius: 'var(--rmd)', backgroundColor: n.read ? 'transparent' : 'var(--bs)', border: n.read ? 'none' : 'var(--bw) var(--bs2) var(--b)', cursor: 'pointer', transition: 'background 150ms' }}>
                {/* Icon */}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '8px', marginBottom: '3px' }}>
                    <p style={{ fontSize: '13px', fontWeight: n.read ? 400 : 600, color: 'var(--tx)', lineHeight: 1.3 }}>{n.title}</p>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
                      <span style={{ fontSize: '10px', color: 'var(--mu)', whiteSpace: 'nowrap' }}>{n.time}</span>
                      {!n.read && <div style={{ width: 6, height: 6, borderRadius: '50%', backgroundColor: 'var(--b)' }} />}
                    </div>
                  </div>
                  <p style={{ fontSize: '12px', color: 'var(--mu)', lineHeight: 1.5 }}>{n.body}</p>
                </div>
              </div>
            ))}
          </div>
