...
export default function GlobalRightPanel() {
  ...
  return (
    <div className={`border-s border-white/[0.05] bg-white/[0.01] backdrop-blur-md flex flex-col transition-all duration-300 ease-in-out ${isCollapsed ? 'w-12' : 'w-96'} h-screen sticky top-0`}>
      {/* Toggle Button */}
      <button 
        onClick={() => setIsCollapsed(!isCollapsed)}
        className="p-3 border-b border-white/[0.05] hover:bg-white/[0.02] text-ds-text-muted hover:text-white transition-colors flex justify-center h-14 items-center"
