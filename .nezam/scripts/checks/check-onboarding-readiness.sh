...
if [[ "$missing" -ne 0 ]]; then
  ...
  echo "Required order:"
  echo "1. /START repo"
  echo "2. Run /START → drop PRD into docs/start/ → agent rewrites it to $prd_path"
  echo "3. Optional: /CREATE prd for guided drafting"
  echo "4. Agent creates $prompt_path automatically during /START"
  echo "5. Create gate manifest at $gate_manifest_path"
