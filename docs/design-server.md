...
The NEZAM Design Server (`.nezam/design-server/`) is a local Next.js 15 application running on port 4000 that serves as a **human-in-the-loop design decision engine**. It produces two artifacts that gate AI swarm development:
...
cd .nezam/design-server && pnpm dev
...
See [`docs/design-server-audit.md`](docs/design-server-audit.md) for:
...
```
[01 Design] → Start Design Server → /START design
              ↓
