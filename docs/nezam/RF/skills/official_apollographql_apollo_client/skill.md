...
- **Client-Side Apps (`references/integration_client.md`)** - For client-side React applications without SSR (Vite, Create React App, etc.)
- **Next.js App Router (`references/integration_nextjs.md`)** - For Next.js applications using the App Router with React Server Components
- **React Router Framework Mode (`references/integration_react_router.md`)** - For React Router 7 applications with streaming SSR
- **TanStack Start (`references/integration_tanstack_start.md`)** - For TanStack Start applications with modern routing
...
- TypeScript Code Generation (`references/typescript_codegen.md`) - GraphQL Code Generator setup for type-safe operations
- Queries (`references/queries.md`) - useQuery, useLazyQuery, polling, refetching
- Suspense Hooks (`references/suspense_hooks.md`) - useSuspenseQuery, useBackgroundQuery, useReadQuery, useLoadableQuery
...
- State Management (`references/state_management.md`) - Reactive variables, local state
- Error Handling (`references/error_handling.md`) - Error policies, error links, retries
