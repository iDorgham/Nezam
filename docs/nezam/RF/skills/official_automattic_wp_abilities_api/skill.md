...
If you need a logical grouping, register an ability category early (see `references/php_registration.md`).
...
Use the documented init hooks for Abilities API registration so they load at the right time (see `references/php_registration.md`).

### 5) Confirm REST exposure

- Verify the REST endpoints exist and return expected results (see `references/rest_api.md`).
...
- For canonical details, consult:
  ...
  - `references/php_registration.md`
