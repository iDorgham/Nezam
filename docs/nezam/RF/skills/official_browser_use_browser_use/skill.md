...
For advanced browser control (CDP, device emulation, tab activation), see `references/cdp_python.md`.
...
For subagent workflows or running multiple browsers in parallel, use `--session NAME`. Each session gets its own browser. See `references/multi_session.md`.
