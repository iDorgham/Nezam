...
3. **Look up the LSP server** — read `references/lsp_servers.md` for known servers, install commands, and config snippets
...
- If the language is not listed in `references/lsp_servers.md`, search the web for "<language> LSP server" and guide the user through manual configuration.
