...
- **`references/batching_categories.md`** - Category A, B, C with full before/after code
- **`references/flushSync_guide.md`** - when to use flushSync, when NOT to, import syntax
...
In most cases, the fix is a **refactor** - restructuring the code to not read `this.state` after `await`. Read `references/batching_categories.md` for the correct approach per category.
