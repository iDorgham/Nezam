...
1. Reads the current task from a baton file (`.stitch/next_prompt.md`)
...
The `.stitch/next_prompt.md` file acts as a relay baton between iterations:
...
Parse `.stitch/next_prompt.md` to extract:
...
**You MUST update `.stitch/next_prompt.md` before completing.** This keeps the loop alive.
...
│   ├── next_prompt.md  # The baton — current task
...
| Method | How it works |
|--------|--------------|
| **CI/CD** | GitHub Actions triggers on `.stitch/next_prompt.md` changes |
...
- ❌ Forgetting to update `.stitch/next_prompt.md` (breaks the loop)
...
| Loop stalls | Verify `.stitch/next_prompt.md` was updated with valid frontmatter |
