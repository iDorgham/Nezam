...
See references/MIGRATION_LIBRARY.md (`references/MIGRATION_LIBRARY.md`) for full before/after code.
...
See references/MIGRATION_APP_SPLIT.md (`references/MIGRATION_APP_SPLIT.md`) for full guide.
...
See references/MIGRATION_FULL_RESTRUCTURE.md (`references/MIGRATION_FULL_RESTRUCTURE.md`) for full guide.
...
See references/PLUGIN_COMPATIBILITY.md (`references/PLUGIN_COMPATIBILITY.md`) for the full compatibility table with known compatible versions, opt-out flag workarounds, and broken plugins.
...
See references/KNOWN_ISSUES.md (`references/KNOWN_ISSUES.md`) for details. Key gotchas:
...
- DSL Reference (`references/DSL_REFERENCE.md`) — side-by-side old→new DSL mapping
- Version Matrix (`references/VERSION_MATRIX.md`) — AGP/Gradle/KGP/Compose/IDE compatibility
- Plugin Compatibility (`references/PLUGIN_COMPATIBILITY.md`) — third-party plugin status and workarounds
