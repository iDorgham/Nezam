...
- **`references/location_tracking.md`** -- Camera follow user location + get current location once
- **`references/custom_data.md`** -- GeoJSON sources and layers: lines, polygons, points, update/remove
- **`references/camera_styles.md`** -- Camera control (set, animate, fit) + map styles (built-in and custom)
