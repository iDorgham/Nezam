...
5. **Set up tunneling** (local dev) — Use Tailscale Funnel (recommended) or ngrok. See references/webhook_setup.md (`references/webhook_setup.md`)
6. **Create webhook via API** — Use the Resend Webhook API to register your endpoint programmatically. See references/webhook_setup.md (`references/webhook_setup.md`)
...
For detailed implementation code for each level, see references/security_levels.md (`references/security_levels.md`).
...
For webhook registration via API, tunneling setup, svix fallback, and retry behavior, see references/webhook_setup.md (`references/webhook_setup.md`).
...
| No rate limiting | Implement per-sender rate limits. See references/advanced_patterns.md (`references/advanced_patterns.md`) |
