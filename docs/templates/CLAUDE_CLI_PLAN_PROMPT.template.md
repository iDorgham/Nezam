...
2. `docs/prompts/PROJECT_PROMPT.md`
3. `docs/context/instructions.md`
4. `docs/context/workspace.md`
...
1. **SEO research** — Create or refresh `docs/specs/sdd/SEO_RESEARCH.md` (keywords/clusters, intents, snippets, FAQ seeds, slug rules).
2. **Information architecture** — Only after SEO exists: menus, URL map, breadcrumbs, naming driven by SEO tables; document under `docs/specs/sdd/` (extend README or add `IA.md` if no dedicated file yet).
...
4. **Design brief** — Only after content outlines exist: create or refresh root **`DESIGN.md`** with layout paradigm (one-page, blog, corporate, portfolio, gallery, storefront; scroll paradigm), palette, typography, motion, accessibility notes, **textual example screens** developers can mirror.
5. **Versioning** — Fill or refresh `docs/specs/sdd/VERSIONING.md` (semver, changelog policy, tags `vMAJOR.MINOR.PATCH`, Conventional Commits, release branches).
6. **Roadmap / phases / architecture / data** — Under `docs/specs/sdd/` create or evolve:
   - `ROADMAP.md`
   - `PHASES.md`
   - `ARCHITECTURE.md`
...
- Recommended next step in Cursor: **`/PLAN all`** or targeted **`/PLAN seo`** ... for refinement, then **`/DEVELOP`** after `DESIGN.md` sign-off analog.
