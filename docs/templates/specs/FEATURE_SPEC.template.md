...
> API bindings, props structure, and state variations for design system components.

| Component | Prop Name | Type | Allowed Values | Default | Description |
|---|---|---|---|---|---|
| `{{ComponentName}}` | `variant` | string | `default` \| `primary` \| `secondary` \| `danger` | `default` | Style variation |
| `{{ComponentName}}` | `size` | string | `sm` \| `md` \| `lg` | `md` | Spacing scale |
| `{{ComponentName}}` | `isDisabled` | boolean | `true` \| `false` | `false` | Disabled state |

---

## Motion and reduced-motion contract

> Defines the fine-grained animation transitions, durations, easing curves, and screen-reader accessibility / reduced-motion fallback options.

| Interaction | Animation | Duration | Easing | GPU Hint | Reduced-motion fallback |
|---|---|---|---|---|---|
| {{INTERACTION}} | {{ANIMATION_TYPE}} | {{MS}}ms | `cubic-bezier(...)` | `will-change` | instant / fade only |
