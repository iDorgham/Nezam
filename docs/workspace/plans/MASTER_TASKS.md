...
## Task Routing Schema (required)

Tasks generated by `/PLAN tasks` or `/CREATE task` should include:

```yaml
- id: MT-###
  title: "..."
  type: documentation | scan | boilerplate | architecture-decision | security | other
  assigned_tool: claude | cursor | gemini | qwen | kilocode | opencode | copilot | codex | antigravity
  fallback_tool: <tool-or-null>
  security: true|false
  original_tool: <tool-or-null>
  status: pending | in-progress | in-review | done | blocked | deferred
```
